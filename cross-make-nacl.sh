...
SHARED_OBJECTS="build/release-nacl-$ARCH/renderer_rend2_$ARCH.so build/release-nacl-$ARCH/renderer_opengl1_$ARCH.so"
$NACL_SDK_ROOT/tools/create_nmf.py -D $NACL_SDK_ROOT/toolchain/linux_x86_glibc/bin/i686-nacl-objdump $EXE -L $NACL_SDK_ROOT/toolchain/linux_x86_glibc/x86_64-nacl/lib -L $NACL_SDK_ROOT/toolchain/linux_x86_glibc/x86_64-nacl/usr/lib -o $NMF $SHARED_OBJECTS -s build/release-nacl-$ARCH || echo "creat_nmf failed"
